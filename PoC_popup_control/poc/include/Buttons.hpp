#include "Arduino.h"

class Buttons
{
private:
    const gpio_num_t _pin;
    bool _lastState;

public:
    Buttons(gpio_num_t pin);
    ~Buttons();
    bool isPressed(bool debug = false);
    void *onPress(void *(*callback)(void *), void *data = nullptr);
    void *onChange(void *(*callback)(void *), void *data = nullptr);
};

Buttons::Buttons(gpio_num_t pin)
    : _pin(pin)
{
    pinMode(_pin, INPUT_PULLUP);
}

Buttons::~Buttons()
{
}

bool Buttons::isPressed(bool debug)
{
    if (debug)
    {
        bool pressed = !(bool)digitalRead(_pin);
        Serial.printf("B[%d]:(%s)\n", _pin, pressed ? "pressed" : "not pressed");
        return pressed;
    }
    return !(bool)digitalRead(_pin);
}

void *Buttons::onPress(void *(*callback)(void *), void *data)
{
    if (this->isPressed())
    {
        return callback(data);
    }
    return nullptr;
}

void *Buttons::onChange(void *(*callback)(void *), void *data)
{
    bool currentState = this->isPressed();
    if (currentState != _lastState)
    {
        void *result = callback(data);
        _lastState = currentState;
        return result;
    }
    return nullptr;
}